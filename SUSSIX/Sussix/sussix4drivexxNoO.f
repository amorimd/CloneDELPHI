...
C
     ...
     &turnnumber
      double precision eps,etune,tunex,tuney,tunez, xy(4*turnnumber), 
     &tunexy(2), 
     & amplitude(14), phase(14), ox(600), ax(600), oy(600), 
     &ay(600)

Cf2py intent(in) xy
Cf2py intent(out) tunexy
...
C=========================
      ...
      open(10,file='sussix_v4.inp',form='formatted',status='unknown')
      read(10,'(4(/))')
      read(10,'(A)') ch
      ch1=ch(9:80)//' / '
      read(ch1,*)isix
...
C                close(30)
     ...
     &             imeth,narm,nrc,iana,turnnumber)
...
C=======================================================================
     ...
     &amplitude(14), phase(14), ox(600), ax(600),
     &oy(600), ay(600)
...
C          write(6,*)'X TUNE DIFFERENT FROM EXPECTED'
C          write(6,*)-tx(1),-tunex
          ...
          enddo
 7        if(ntx.gt.1) then
C            write(6,*)'EXPECTED TUNE X FOUND AT LINE',ntx
          elseif(ntx.eq.1) then
C            write(6,*)'EXPECTED TUNE X NOT FOUND'
...
C              write(6,*)'EXPECTED TUNE S NOT FOUND'
C              if(istune.eq.1) write(6,*)'LINE 1 ASSUMED AS TUNE!!!'
            endif
            tz(1)=tz(ntz)
            tz(ntz)=tsa(1)
...
c...  Insertion, to return parameters
            if(n.eq.1) then
              amplitude(4)=py
c              write(*,*)"p3 ", fy, py, ty(n), tunexy(1)
              phase(4)=-fy
              tunexy(2)=-ty(n)
            endif
...
C            write(30,100)n,-ty(n),py,0,0
      ...
      do myint=1,14
         if(flagad(myint).eq.0) then
            ...
            if(myint.eq.4) then
               tunexy(2)=0
C               write(*,*)"VERTICAL TUNE NOT IDENTIFIED (in sussix)"
            endif
         endif
      enddo
...
C=======================================================================
      implicit none
      integer iana,idam,imeth,ir,iunit,j,k,turnnumber,
     &maxn,mterm,narm,nrc,nt1,nt2,nturn,nturn2
      double precision duepi,xy(4*turnnumber),eps
      complex zsing
      parameter(mterm=600)
      parameter(maxn=100000)
      double precision s,sp,x,xp,y,yp
...
C          read(iunit,*,end=990)x(j),xp(j),y(j),yp(j)
           x(j)=xy(j)
           xp(j)=xy(j+2*turnnumber)
           y(j)=xy(j+turnnumber)
           yp(j)=xy(j+3*turnnumber)
        enddo
