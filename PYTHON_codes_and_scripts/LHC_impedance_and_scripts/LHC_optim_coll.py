...
if out.startswith('lxplus'):
    sys.path.insert(1,'/afs/cern.ch/user/n/nmounet/private/soft/Pymodules/numpy-1.7.0/build/lib.linux-x86_64-2.6');
    sys.path.insert(1,'/afs/cern.ch/user/n/nmounet/private/soft/Pymodules/scipy-0.12.0/build/lib.linux-x86_64-2.6');
...
if __name__ == "__main__":
	...
	if (lxplusbatchDEL==None)or(lxplusbatchDEL.startswith('retrieve')):
	    ...
	    # parameters
	    oct2012=250;Nb2012=1.5e11;emit2012=2.5;en2012=4;
		...
		for idamp,damp in enumerate(dampscan):
		    ...
		    fileoutplotemit=root_result+'/plot_int_vs_emit_'+machine+'_'+float_to_str(round(E/1e9))+'GeV'+'_'+str(M)+'b_d'+float_to_str(damp)+'_Qp'+float_to_str(Qpaver[0])+'_'+float_to_str(Qpaver[-1])+'_converged'+strnorm[flagnorm];
		    ...
		    factor2012lin=oct2012*emit2012/(en2012*Nb2012/1e11);
		    # better : oct*emit/en^2 =  fact *imag(-tuneshift(int));
		    #factor2012=oct2012*emit2012/(en^2*imag(-tuneshift));
		    imtu0=np.min(getattr(tuneshiftQp[0,:,iM,iQpaver,idamp,pylab.mlab.find(Nbscan==Nb2012),0,0,0],'imag'));
		    print imtu0
		    factor2012=oct2012*emit2012/(-en2012**2*imtu0);
			...
			fileoutdataemit=root_result+'/data_int_vs_emit_'+machine+'_'+float_to_str(round(E/1e9))+'GeV'+scenario+'_'+str(M)+'b_d'+float_to_str(damp)+'_Qp'+float_to_str(Qpaver[0])+'_'+float_to_str(Qpaver[-1])+'_converged'+strnorm[flagnorm];
			...
			emitLS1lin=np.max(maxratio[:,iscenario,:])*factor2012lin*enLS1/octLS1 * (Nbscan/1e11);
