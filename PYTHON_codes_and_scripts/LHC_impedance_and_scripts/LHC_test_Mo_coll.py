...
if __name__ == "__main__":
    ...
    fig,ax=init_figure(); # figure for impedance ratio
	...
	namesref=select_LHC_coll_IR(namestot,pattern='TCS',IRlist=[7]);
	# anti-selection
	namesanti=invert_selection(namestot,namesref)
	#print namesanti,namesref
	...
	if iscenario==0:
	    ...
	    imp_mod_coll,wake_mod_coll=LHC_manycoll_iw_model(E,avbetax,avbetay,param_filename_coll,settings_filename_coll,
		beta_filename_coll,wake_calc=wake_calc,ftypescan=0,nflog=100,namesref=namesref,coatingmat='Mo',coatingthickness=50e-6,
		lxplusbatch=lxplusbatch,comment=model+scenario,dire='Coll'+model+scenario+'/');
			
	imp_mod_coll2,wake_mod_coll2=LHC_manycoll_iw_model(E,avbetax,avbetay,param_filename_coll,settings_filename_coll,
	    beta_filename_coll,wake_calc=wake_calc,ftypescan=0,nflog=100,namesref=namesanti,
	    lxplusbatch=lxplusbatch,comment=model+scenario,dire='Coll'+model+scenario+'/');	
	...
	add_impedance_wake(imp_mod,imp_mod_coll2,1,1);
	add_impedance_wake(wake_mod,wake_mod_coll2,1,1);
	...
	if (lxplusbatch.startswith('retrieve'))or(lxplusbatch==None):
		...
		for ir,r in enumerate(['Re','Im']):
		    #print comp,iw_comp.plane,iw_comp.a,iw_comp.b;
        	    plot(imp_mod[kiw_comp].var,np.abs(imp_mod[kiw_comp].func[:,ir]),r+'('+comp+')'+scenario.replace('_',' '),mark[iscenario]+pat[ir]+col[icomp],"Z [$\Omega$/m]",ax,3,xlab='Frequency [Hz]');

    	    #if flagsave: end_figure(fig,ax,save=root_result+'/plot_imp_'+machine+model+scenario)
		...
		for icomp,comp in enumerate(listcomp[:2]):
		    ...
		    # plot
        	    plot(imp_mod[kiw_comp].var,np.abs(imp_mod[kiw_comp].func[:,0]+1j*imp_mod[kiw_comp].func[:,1])/np.abs(imp_mod0[kiw0_comp].func[:,0]+1j*imp_mod0[kiw0_comp].func[:,1]),comp+', post LS1,'+scenario.replace('_',' '),pat[icomp]+col[iscenario],"Imp. ratio",axratio,1,xlab='Frequency [Hz]');


    ax.set_xlim([1e3,1e11]);ax.set_ylim([1e4,1e10]);
