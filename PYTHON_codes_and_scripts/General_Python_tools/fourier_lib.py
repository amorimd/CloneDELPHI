...
def fourier_integral_any_sampling(tscan,omegai,fi,df=0.,interp=0,flaginf=1,eps=1e-20,di=None):
    ...
    pchipslopes = prototypepchip(('pchipslope', libIW2D));
    ...
    # array for pchip slopes
    flagdi=False; # case when pchip slopes are already computed
    if (di==None):
	...
	pchipslopes(di_arr,omegai_arr,fi_arr,c_ulong(n));
...
def compute_intercalate_pchipslope(i,omegai,fi,di):
    ...
    # and intercalate the slope at the middle point.
    offset=min(0,i-1); # offset of -1 in case i=0 (because then we cannot
    # fetch an additional point on the left, to compute the slopes)
...
def bisect_and_calculate(f,i,omegai,fi,di,bisect_type='log',pchip_flag=True):
	...
	dinew=compute_intercalate_pchipslope(i,omegainew,finew,di)
...
def fourier_int_convergence_boundary(f,df,omegabound,tscan,omegai,fi,di,omegainew,finew,dinew,min_or_max,
	...
	if pchip_flag: di=compute_intercalate_pchipslope(i,omegai,fi,di);
	...
	if pchip_flag: dinew=compute_intercalate_pchipslope(inew,omegainew,finew,dinew)
	else: dinew=add_element_in_array(dinew,inew,0.);
	
	# subtract old asymptotic part if needed
	if (flaginf==0):fintnew=deepcopy(fint);
	else: fintnew=fint - fourier_integral_asympt_correction(sign(omegab),tscan,omegab,f(omegab),df_end=df(omegab));
...
def fourier_int_converged(f,df,tscan,omegamin,omegamax,tolabs=1e10,tolrel=1e-3,
	...
	if np.isinf(omegamin): iinf=0;flaginf=sign(omegamin);
	else: iinf=-1;flaginf=sign(omegamax);
    else: flaginf=0;
    ...
    # final temporary checks (on the pchipslopes and fi)
