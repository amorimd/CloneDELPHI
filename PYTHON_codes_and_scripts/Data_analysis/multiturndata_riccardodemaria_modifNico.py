...
class MultiTurnData(object): # for ADT "Verena kind of data" (i.e. from the multiturn application)
  ...
  def __init__(self,fn):
	...
	# pad with zeros if array is truncated for some reason (+ disp. Warning)
	if (len(arr)<self.bunches*self.turns):
	   arrnew=np.zeros(self.bunches*self.turns,dtype=float);
	   for i,el in enumerate(arr): arrnew[i]=el;
	   arr=arrnew;
	   print "WARNING: ", deviceName," data truncated, pad with zeros"
